---
title: |
  | Model selection bias invalidates
  | goodness of fit tests
subtitle: "When the best isn't good enough"
author: "Joshua Loftus and Weichi Yao"
institute: "New York University"
output: 
  beamer_presentation:
    includes:
      in_header: nyu_header.tex
    theme: "Boadilla"
    colortheme: "seahorse"
    toc: true
    fig_caption: false
...
- "Model specification test," "omitted variable bias"
...
## Example: follow-up study after Benjamini-Hochberg
...
- Goodness of fit in high dimensions: Verzelen & Villers (2008), Nan & Yang (2014), Shah & Bühlmann (2017)

## Conditional on selection literature
...
- As before, but now $n = 100$ observations and $p = 200$ variables
- Can't use AIC/BIC, so we use RIC (Foster & George, 1994), i.e. in \texttt{R} with \texttt{step(..., k = 2log(p))}.
- Simulation with $n = 100$ observations of $p = 10$ variables, the first two coefficients are larger than the next 3, and the last 5 are all 0.
- Can't use full model as the alternative, so we'll find $A'$ by letting forward stepwise add the next 5 variables


## Profile of model selection events
  \centering
  \includegraphics[width=.99\textwidth]{fstepric1.pdf}
...
\caption{\label{tab:1}Probability of rejection at level 0.1}
\centering
\begin{tabular}[t]{l|r|r}
\hline
pvalue & overlap & Pr(reject)\\
...
Conditioning on those choices and using truncated distributions is a simple idea with wide applicability, including to goodness of fit tests
