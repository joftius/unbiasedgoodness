...
instance <- function(n, p, s0) {
  ...
  list(
    null = length(intersect(gof_fit$selected_support, 1:s0)) == s0,
    alt = length(intersect(union(gof_fit$selected_support, gof_fit$alt_support), 1:s0)) == s0,
    pval = gof_fit$pval
    )
}

output <- replicate(niters, instance(n, p, s0))
output
```
